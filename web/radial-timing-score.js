...
(function(root){
  ...
  function scoreName(sections,arrangements){
    ...
    const full=parts.join(' ⇒ ')||'Timing Score';
  ...
  function createScore(sections,arrangements,recipes,maximum){
    const safe=sanitizeSections(sections);
    ...
    const missingArrangements=[],missingSources=[],changedSources=[],resolvedSections=[];
    let rawLength=0;
  ...
  function viewFor(environment){
    ...
    if(project&&(typeof project==='object'||typeof project==='function')){
      ...
      if(!view){view={open:false,selectedArrangementId:null,sections:[],preview:false,name:''};projectViews.set(project,view);}
      return view;
    }
    return fallbackView;
  }
  function arrangementLibrary(){
    const library=root.InkFrameRadialPhraseLibrary;
    return library&&library.store&&typeof library.store.snapshot==='function'?library.store.snapshot().phrases:[];
  }
  function recipeLibrary(){
    const recipes=root.InkFrameRadialRecipes;
    return recipes&&recipes.store&&typeof recipes.store.snapshot==='function'?recipes.store.snapshot().recipes:[];
  }
  function ensureState(environment,view){
    const arrangements=arrangementLibrary();
    if(!arrangements.some(item=>item.id===view.selectedArrangementId))view.selectedArrangementId=arrangements[0]&&arrangements[0].id||null;
    view.sections=Array.from(sanitizeSections(view.sections),section=>({...section}));
    const score=createScore(view.sections,arrangements,recipeLibrary(),MAX_VALUES);
    if(!score||!score.valid)view.preview=false;
    return {arrangements,score};
  }
  ...
  function structureSnapshot(project){
    const snapshot=viewSnapshot(project);
    return Object.freeze({name:snapshot.name,selectedArrangementId:snapshot.selectedArrangementId,sections:snapshot.sections});
  }
  ...
  function mutateSections(mutator){
    ...
    const view=viewFor(lastEnvironment),state=ensureState(lastEnvironment,view),next=view.sections.map(section=>({...section}));
    const result=mutator(next,state);
    if(result===false)return false;
    view.sections=Array.from(sanitizeSections(next),section=>({...section}));
    scheduleRefresh(false);
    return true;
  }
  function addSelected(){
    return mutateSections((sections,state)=>{
      const view=viewFor(lastEnvironment);
      if(!view.selectedArrangementId||sections.length>=MAX_SECTIONS||!state.arrangements.some(item=>item.id===view.selectedArrangementId))return false;
      sections.push({arrangementId:view.selectedArrangementId,repeat:1});
    });
  }
  ...
  function moveSection(index,delta){
    return mutateSections(sections=>{
      const from=Math.floor(finite(index,-1)),to=Math.max(0,Math.min(sections.length-1,from+Math.floor(finite(delta,0))));
      if(!sections[from]||from===to)return false;
      const [section]=sections.splice(from,1);sections.splice(to,0,section);
    });
  }
  ...
  function clearSections(){
    if(!lastEnvironment||!canEdit(lastEnvironment))return false;
    const view=viewFor(lastEnvironment);
    if(!view.sections.length)return false;
    view.sections=[];view.preview=false;scheduleRefresh(false);return true;
  }
  function loadStructure(structure){
    if(!lastEnvironment||!canEdit(lastEnvironment))return false;
    const input=structure&&typeof structure==='object'?structure:{},view=viewFor(lastEnvironment),arrangements=arrangementLibrary(),sections=sanitizeSections(input.sections);
    if(!sections.length)return false;
    if(sections.some(section=>!arrangements.some(item=>item.id===section.arrangementId)))return false;
    view.sections=Array.from(sections,section=>({...section}));
    view.name=cleanName(input.name)||scoreName(sections,arrangements);
    const selectedId=String(input.selectedArrangementId||sections[0].arrangementId||'');
    view.selectedArrangementId=arrangements.some(item=>item.id===selectedId)?selectedId:sections[0].arrangementId;
    view.preview=false;
    scheduleRefresh(false);
    return true;
  }
  function assignmentsForScore(environment,score){
    ...
    if(!patterns||!score||!score.valid)return Object.freeze([]);
  ...
  function applyScore(){
    if(!lastEnvironment||!canEdit(lastEnvironment))return false;
    const patterns=root.InkFrameRadialPatterns,view=viewFor(lastEnvironment),state=ensureState(lastEnvironment,view);
  ...
  function saveScore(){
    ...
    const recipes=root.InkFrameRadialRecipes,view=viewFor(lastEnvironment),state=ensureState(lastEnvironment,view);
    if(!recipes||!state.score||!state.score.valid)return null;
    const saved=recipes.store.save(cleanName(view.name)||state.score.label,state.score.values);
    view.name=saved.name;scheduleRefresh(true);return saved;
  }

  function installStyle(document){
    if(styleInstalled||!document||!document.head)return;
    const style=document.createElement('style');style.dataset.inkframeScoreStyle='true';
    style.textContent=`
...
`;
  ...
  function createPreview(document,board,plan,environment,score){
    const timing=root.InkFrameRadialTiming,assignments=assignmentsForScore(environment,score);
    if(!timing||!assignments.length)return;
    ...
    for(const entry of assignments){
      ...
      const path=document.createElementNS('http://www.w3.org/2000/svg','path');path.classList.add('inkframe-score-preview-arc');path.dataset.frame=String(entry.index);path.dataset.hold=String(entry.after);path.setAttribute('d',timing.holdArcPath(plan,slot,entry.after));svg.appendChild(path);
    }
    board.appendChild(svg);
  }
  function createShelf(document,board,environment,view,state){
    const patterns=root.InkFrameRadialPatterns,scope=patterns.resolveTargetIndices(environment),score=state.score,shelf=document.createElement('div');
    shelf.className='inkframe-score-shelf';shelf.setAttribute('role','toolbar');shelf.setAttribute('aria-label','Timing score composer');
    const status=document.createElement('span');status.className='inkframe-score-status';
    status.textContent=!score?'Add saved phrase arrangements to build a score':!score.valid?`${score.sections.length} sections · ${score.missingArrangements.length+score.missingSources.length} unresolved`:`${score.sections.length} sections · ${score.values.length} steps${score.truncated?' · capped':''}${score.changedSources.length?` · ${score.changedSources.length} changed source${score.changedSources.length===1?'':'s'}`:''} · ${scope.label}`;
    ...
    for(const record of state.arrangements){
      const option=document.createElement('option');option.value=record.id;option.textContent=record.name;option.selected=record.id===view.selectedArrangementId;select.appendChild(option);
    }
    select.addEventListener('change',event=>{event.stopPropagation();if(!canEdit(lastEnvironment)){scheduleRefresh(false);return;}view.selectedArrangementId=select.value;scheduleRefresh(false);});shelf.appendChild(select);
    ...
    score&&score.sections.forEach((section,index)=>{
      const record=state.arrangements.find(item=>item.id===section.arrangementId),resolution=record?resolveArrangement(record,recipeLibrary()):null,group=document.createElement('div');
      group.className='inkframe-score-section';group.setAttribute('role','listitem');group.dataset.section=String(index);
  ...
  function installBoard(board){
    if(!board||board._inkframeScoreInstalled)return;
    board._inkframeScoreInstalled=true;
    if(typeof root.MutationObserver==='function'){
      const observer=new root.MutationObserver(()=>{
        if(rendering||refreshQueued||!lastBoard||board!==lastBoard)return;
        const phraseShelf=board.querySelector('.inkframe-phrase-shelf'),missing=phraseShelf&&!phraseShelf.querySelector('.inkframe-score-toggle'),stale=!phraseShelf&&(board.querySelector('.inkframe-score-shelf')||board.querySelector('.inkframe-score-preview-svg'));
  ...
  function render(board,environment){
    ...
    if(!board||!document||!plan||!phrases||!phraseLibrary||!patterns)return false;
    ...
    try{
      ...
      const phraseShelf=board.querySelector('.inkframe-phrase-shelf'),view=viewFor(lastEnvironment);
      if(!phraseShelf){view.open=false;view.preview=false;return true;}
      const state=ensureState(lastEnvironment,view),toggle=makeButton(document,'Score','inkframe-score-toggle',()=>{view.open=!view.open;scheduleRefresh(false);});
      toggle.setAttribute('aria-pressed',view.open?'true':'false');toggle.title='Arrange saved timing phrases into a complete score';phraseShelf.appendChild(toggle);
      if(view.open)createShelf(document,board,lastEnvironment,view,state);
      if(view.open&&view.preview&&state.score&&state.score.valid)createPreview(document,board,plan,lastEnvironment,state.score);
      return true;
    }finally{rendering=false;}
  }
  function installIntoRadial(){
    const radial=root.InkFrameRadialTimeline;
    if(!radial||radial.__radialScorePatched)return false;
    ...
    radial.__radialScorePatched=true;
    return true;
  }
  ...
  const api={
    ...
    arrangementLibrary,recipeLibrary,structureSnapshot,loadStructure,assignmentsForScore,addSelected,setSectionRepeat,moveSection,duplicateSection,removeSection,clearSections,applyScore,saveScore,render,viewSnapshot,installIntoRadial,
