...
try {
  ...
  assert.ok(existsSync(resolve(root,'web/brush-engine-v2/profile-history.js')),'missing Profile History asset');
  ...
  assert.ok(compareSource.includes("script.src='brush-engine-v2/profile-history.js'"),'comparison runtime must load Profile History');
  ...
  assert.ok(compareSource.includes("script.addEventListener('load',()=>root.setTimeout(loadProfileHistory,0)"),'Profile History must be scheduled from the Calibration Report load event');
  ...
  assert.ok(compareSource.includes('script[data-inkframe-profile-history]'),'Profile History loader must suppress duplicates');
  ...
  console.log('✅ generated Brush V2 production Profile History policy passed');
} finally {
