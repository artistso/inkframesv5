...
assert.match(overlay,/if \(overlay\.hasHover\) overlay\.cancelHover\(\)/,'hover must release before surrounding controls receive the S Pen');
assert.match(overlay,/if \(showHover\) drawArtistStatus\(canvas\)/,'the artist HUD must draw after the artwork clip is restored');
