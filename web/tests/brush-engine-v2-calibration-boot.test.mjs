...
try{
  ...
  html=html.replace('</body>',`<script>${replay}</script><script>${coach}</script><script>${sessionSource}</script><script>${reportSource}</script></body>`);
  ...
  const d=dom.window.document,preview=dom.window.InkFrameBrushV2PreviewPad,replayApi=dom.window.InkFrameBrushV2ReferenceReplay,coachApi=dom.window.InkFrameBrushCoach,sessionApi=dom.window.InkFrameBrushCoachSession,reportApi=dom.window.InkFrameBrushCalibrationReport;
  assert.ok(coachApi&&coachApi.installed,'Brush Coach did not install');assert.ok(sessionApi&&sessionApi.installed,'Coach Session did not install');assert.ok(reportApi&&reportApi.installed,'Calibration Report did not install');
  ...
  await stroke(61,[[40,120,.25,100],[55,116,.35,145],[70,122,.45,190],[86,118,0,235]]);await stroke(62,[[60,160,.3,300],[120,80,.45,316],[180,160,.55,332],[240,80,.65,348],[300,160,0,364]]);await stroke(63,[[30,130,.35,400],[210,55,.5,406],[420,145,.65,412],[650,70,0,418]]);await stroke(64,[[50,130,.08,500],[150,110,.25,520],[250,125,.5,540],[350,105,.82,560],[470,130,0,580]]);
  const sessionSuggestion=sessionApi.model.suggestion();assert.equal(sessionApi.model.snapshot().completed,4);assert.equal(sessionSuggestion.valid,true);const sessionButtons=Array.from(sessionApi.details.querySelectorAll('.inkframe-v2-coach-session-actions button'));assert.ok(sessionButtons.slice(0,3).every(button=>!button.disabled));assert.equal(sessionApi.compare(),true);await new Promise(r=>setTimeout(r,40));assert.ok(store.snapshot().presets.some(item=>item.name==='Coach · Complete Session'));assert.ok(replayApi.stats().lastReplay.b>0);
  const report=reportApi.render();assert.equal(report.ready,3);assert.equal(report.rows.length,8);assert.equal(reportApi.details.querySelectorAll('.inkframe-v2-calibration-profile').length,3);assert.equal(reportApi.select('session'),true);const reportBefore=dom.window.InkFrameBrushV2Adapter.currentTuning();assert.equal(reportApi.compare(),true);await new Promise(r=>setTimeout(r,40));assert.ok(store.snapshot().presets.some(item=>item.name==='Calibration · Full Session'));assert.ok(replayApi.stats().lastReplay.b>0);assert.equal(reportApi.apply(),true);const reportAfter=dom.window.InkFrameBrushV2Adapter.currentTuning();assert.equal(reportAfter.stabilizerStrength,sessionSuggestion.tuning.stabilizerStrength);assert.notEqual(reportAfter.stabilizerStrength,reportBefore.stabilizerStrength);reportApi.save();assert.equal(store.snapshot().presets.filter(item=>item.name==='Calibration · Full Session').length,1,'saving the same report target should update, not duplicate');assert.equal(sessionApi.apply(),true);
  assert.equal(coachApi.projectCanvasWrites,0);assert.equal(coachApi.undoWrites,0);assert.equal(sessionApi.projectCanvasWrites,0);assert.equal(sessionApi.undoWrites,0);assert.equal(reportApi.projectCanvasWrites,0);assert.equal(reportApi.undoWrites,0);assert.equal(dom.window.__canvasOps.filter(x=>x.canvas===main).length,mainBefore);sessionButtons[3].click();assert.equal(sessionApi.model.snapshot().completed,0);assert.equal(reportApi.render().ready,2);
  dom.window.close();console.log('✅ generated Android Coach calibration report is explicit and project-isolated');
