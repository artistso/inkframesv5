...
const layer={count:3,active:2,background:false,visible:true,opacity:100,blend:'Normal',canInteract:true};
w.InkFrameTabletDeckEnvironment=()=>({
  ...
  layerCommand:(name,value)=>{
    ...
    if(name==='background'){layer.background=true;layer.active=0;}
    else if(name==='selectAbove'&&layer.background){layer.background=false;layer.active=1;}
    else if(name==='selectAbove'&&layer.active<layer.count)layer.active++;
    else if(name==='selectBelow'&&!layer.background&&layer.active===1){layer.background=true;layer.active=0;}
    else if(name==='selectBelow'&&!layer.background&&layer.active>1)layer.active--;
    ...
    else if(layer.background&&['add','duplicate','delete','moveUp','moveDown','mergeDown'].includes(name))return false;
...
assert.equal(panel.querySelector('[data-layer-state="count"]').textContent,'3 frame layers + BG');
...
command('background').click();api.updateState();
assert.deepEqual(commands.at(-1),['background',undefined]);
assert.equal(panel.querySelector('[data-layer-state="position"]').textContent,'Static BG');
assert.equal(command('background').classList.contains('active'),true);
assert.equal(command('selectBelow').disabled,true);
assert.equal(command('selectAbove').disabled,false);
for(const name of ['add','duplicate','delete','moveUp','moveDown','mergeDown'])assert.equal(command(name).disabled,true,`${name} must be disabled for Static BG`);
command('opacity',50).click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="opacity"]').textContent,'50%');assert.equal(command('opacity',50).classList.contains('active'),true);
...
command('selectAbove').click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="position"]').textContent,'1 / 3');assert.equal(command('background').classList.contains('active'),false);
command('selectBelow').click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="position"]').textContent,'Static BG');
command('selectAbove').click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="position"]').textContent,'1 / 3');

command('opacity',100).click();command('visibility').click();command('blend').click();api.updateState();
command('selectAbove').click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="position"]').textContent,'2 / 3');
...
command('duplicate').click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="count"]').textContent,'4 frame layers + BG');assert.equal(panel.querySelector('[data-layer-state="position"]').textContent,'3 / 4');
command('mergeDown').click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="count"]').textContent,'3 frame layers + BG');assert.equal(panel.querySelector('[data-layer-state="position"]').textContent,'2 / 3');
command('delete').click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="count"]').textContent,'2 frame layers + BG');assert.equal(panel.querySelector('[data-layer-state="position"]').textContent,'1 / 2');assert.equal(command('moveDown').disabled,true);assert.equal(command('mergeDown').disabled,true);
command('delete').click();api.updateState();assert.equal(panel.querySelector('[data-layer-state="count"]').textContent,'1 frame layer + BG');assert.equal(command('delete').disabled,true);
...
assert.match(css,/min-height:48px/);assert.match(css,/grid-template-columns:repeat\(3,1fr\)/);assert.match(css,/grid-template-columns:repeat\(4,1fr\)/);assert.match(css,/data-layer-command="delete"/);
...
dom.window.close();console.log('✅ Layer Workspace Static BG selection, properties, navigation, frame-structure lockout, boundaries, and active-stroke lockout passed');
