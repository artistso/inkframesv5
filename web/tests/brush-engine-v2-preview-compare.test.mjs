...

{
  const appended=[];
  const document={
    querySelector:selector=>appended.find(node=>(selector==='script[data-inkframe-reference-replay]'&&node.dataset.inkframeReferenceReplay)||(selector==='script[data-inkframe-brush-coach]'&&node.dataset.inkframeBrushCoach)||(selector==='script[data-inkframe-coach-session]'&&node.dataset.inkframeCoachSession)||(selector==='script[data-inkframe-calibration-report]'&&node.dataset.inkframeCalibrationReport)||(selector==='script[data-inkframe-profile-recovery]'&&node.dataset.inkframeProfileRecovery)||(selector==='script[data-inkframe-profile-recovery-observer]'&&node.dataset.inkframeProfileRecoveryObserver)||(selector==='script[data-inkframe-profile-identities]'&&node.dataset.inkframeProfileIdentities)||(selector==='script[data-inkframe-identity-mixer]'&&node.dataset.inkframeIdentityMixer)||(selector==='script[data-inkframe-brush-match]'&&node.dataset.inkframeBrushMatch)||(selector==='script[data-inkframe-brush-signature]'&&node.dataset.inkframeBrushSignature))||null,
  ...
  appended[7].listeners.load();
  assert.equal(appended.length,9);
  assert.equal(appended[8].src,'brush-engine-v2/brush-match.js');
  assert.equal(appended[8].async,false);
  appended[8].listeners.load();
  assert.equal(appended.length,10);
  assert.equal(appended[9].src,'brush-engine-v2/brush-signature.js');
  assert.equal(appended[9].async,false);
  ...
  assert.equal(sandbox.InkFrameBrushV2.loadBrushMatch(),true);
  assert.equal(sandbox.InkFrameBrushV2.loadBrushSignature(),true);
  assert.equal(appended.length,10,'loader chain must not append duplicate scripts');
}

console.log('✅ Brush Engine V2 deterministic A/B preview, recovery, identity, mixer, Brush Match, and Brush Signature loader chain passed');