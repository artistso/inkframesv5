...

{
  const appended=[];
  const document={
    querySelector:selector=>appended.find(node=>(selector==='script[data-inkframe-reference-replay]'&&node.dataset.inkframeReferenceReplay)||(selector==='script[data-inkframe-brush-coach]'&&node.dataset.inkframeBrushCoach)||(selector==='script[data-inkframe-coach-session]'&&node.dataset.inkframeCoachSession)||(selector==='script[data-inkframe-calibration-report]'&&node.dataset.inkframeCalibrationReport)||(selector==='script[data-inkframe-profile-recovery]'&&node.dataset.inkframeProfileRecovery)||(selector==='script[data-inkframe-profile-recovery-observer]'&&node.dataset.inkframeProfileRecoveryObserver)||(selector==='script[data-inkframe-profile-identities]'&&node.dataset.inkframeProfileIdentities)||(selector==='script[data-inkframe-identity-mixer]'&&node.dataset.inkframeIdentityMixer)||(selector==='script[data-inkframe-brush-match]'&&node.dataset.inkframeBrushMatch))||null,
  ...
  assert.equal(appended.length,9,'loader chain must not append duplicate scripts');
}

console.log('✅ Brush Engine V2 deterministic A/B preview, recovery, identity, mixer, and Brush Match loader chain passed');
