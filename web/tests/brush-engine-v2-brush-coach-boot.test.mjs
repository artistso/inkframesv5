...
try{
  ...
  html=html.replace('</body>',`<script>${replay}</script><script>${coach}</script><script>${sessionSource}</script><script>${reportSource}</script><script>${recoverySource}</script><script>${observerSource}</script><script>${identitySource}</script><script>${mixerSource}</script></body>`);
  ...
  const d=dom.window.document,preview=dom.window.InkFrameBrushV2PreviewPad,replayApi=dom.window.InkFrameBrushV2ReferenceReplay,coachApi=dom.window.InkFrameBrushCoach,sessionApi=dom.window.InkFrameBrushCoachSession,reportApi=dom.window.InkFrameBrushCalibrationReport,recoveryApi=dom.window.InkFrameBrushProfileRecovery,observerApi=dom.window.InkFrameBrushProfileRecoveryObserver,identityApi=dom.window.InkFrameBrushProfileIdentities,mixerApi=dom.window.InkFrameBrushIdentityMixer,adapter=dom.window.InkFrameBrushV2Adapter;
  assert.ok(coachApi&&coachApi.installed,'Brush Coach did not install');assert.ok(sessionApi&&sessionApi.installed,'Coach Session did not install');assert.ok(reportApi&&reportApi.installed,'Calibration Report did not install');assert.ok(recoveryApi&&recoveryApi.installed,'Profile Recovery did not install');assert.ok(observerApi&&observerApi.installed,'Profile Recovery Observer did not install');assert.ok(identityApi&&identityApi.installed,'Creative Brush Identities did not install');assert.ok(mixerApi&&mixerApi.installed,'Identity Mixer did not install');
  ...
  dom.window.close();console.log('✅ generated Android Coach, recovery, creative identities, and Identity Mixer are project-isolated');
