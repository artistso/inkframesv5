...
try{
  ...
  html=html.replace('</body>',`<script>${replay}</script><script>${coach}</script><script>${sessionSource}</script><script>${reportSource}</script><script>${recoverySource}</script></body>`);
  ...
  const d=dom.window.document,preview=dom.window.InkFrameBrushV2PreviewPad,replayApi=dom.window.InkFrameBrushV2ReferenceReplay,coachApi=dom.window.InkFrameBrushCoach,sessionApi=dom.window.InkFrameBrushCoachSession,reportApi=dom.window.InkFrameBrushCalibrationReport,recoveryApi=dom.window.InkFrameBrushProfileRecovery,adapter=dom.window.InkFrameBrushV2Adapter;
  assert.ok(coachApi&&coachApi.installed,'Brush Coach did not install');assert.ok(sessionApi&&sessionApi.installed,'Coach Session did not install');assert.ok(reportApi&&reportApi.installed,'Calibration Report did not install');assert.ok(recoveryApi&&recoveryApi.installed,'Profile Recovery did not install');
  ...
  assert.equal(recoveryApi.restoreLocked(),true);assert.equal(adapter.currentTuning().stabilizerStrength,lockedBaseline.stabilizerStrength);assert.equal(recoveryApi.undoLast(),true);assert.equal(adapter.currentTuning().stabilizerStrength,experimental.stabilizerStrength,'undo must restore the pre-restore brush');
  ...
  assert.equal(recoveryApi.model.snapshot().entries.length<=24,true);assert.equal(recoveryApi.projectCanvasWrites,0);assert.equal(recoveryApi.undoWrites,0);
  ...
  dom.window.close();console.log('✅ generated Android Coach, calibration, profile lock, and recent changes are project-isolated');
