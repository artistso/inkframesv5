...
try{
  ...
  const reportBefore=dom.window.InkFrameBrushV2Adapter.currentTuning();assert.equal(reportApi.compare(),true);await new Promise(r=>setTimeout(r,40));assert.ok(store.snapshot().presets.some(item=>item.name==='Calibration · Full Session'));assert.ok(replayApi.stats().lastReplay.b>0);
  assert.equal(reportApi.apply(),true);const reportAfter=dom.window.InkFrameBrushV2Adapter.currentTuning();assert.equal(reportAfter.stabilizerStrength,sessionSuggestion.tuning.stabilizerStrength);assert.notEqual(reportAfter.stabilizerStrength,reportBefore.stabilizerStrength);
