...
try{
  ...
  const mixerSource=readFileSync(resolve(webDir,'brush-engine-v2/identity-mixer.js'),'utf8');
  const matchSource=readFileSync(resolve(webDir,'brush-engine-v2/brush-match.js'),'utf8');
  html=html.replace('</body>',`<script>${replay}</script><script>${coach}</script><script>${sessionSource}</script><script>${reportSource}</script><script>${recoverySource}</script><script>${observerSource}</script><script>${identitySource}</script><script>${mixerSource}</script><script>${matchSource}</script></body>`);
  ...
  const d=dom.window.document,preview=dom.window.InkFrameBrushV2PreviewPad,replayApi=dom.window.InkFrameBrushV2ReferenceReplay,coachApi=dom.window.InkFrameBrushCoach,sessionApi=dom.window.InkFrameBrushCoachSession,reportApi=dom.window.InkFrameBrushCalibrationReport,recoveryApi=dom.window.InkFrameBrushProfileRecovery,observerApi=dom.window.InkFrameBrushProfileRecoveryObserver,identityApi=dom.window.InkFrameBrushProfileIdentities,mixerApi=dom.window.InkFrameBrushIdentityMixer,matchApi=dom.window.InkFrameBrushMatch,adapter=dom.window.InkFrameBrushV2Adapter;
  assert.ok(coachApi&&coachApi.installed,'Brush Coach did not install');assert.ok(sessionApi&&sessionApi.installed,'Coach Session did not install');assert.ok(reportApi&&reportApi.installed,'Calibration Report did not install');assert.ok(recoveryApi&&recoveryApi.installed,'Profile Recovery did not install');assert.ok(observerApi&&observerApi.installed,'Profile Recovery Observer did not install');assert.ok(identityApi&&identityApi.installed,'Creative Brush Identities did not install');assert.ok(mixerApi&&mixerApi.installed,'Identity Mixer did not install');assert.ok(matchApi&&matchApi.installed,'Brush Match did not install');
  ...
  const mixerButtons=Array.from(mixerApi.details.querySelectorAll('.inkframe-v2-identity-mixer-actions button'));assert.deepEqual(mixerButtons.map(button=>button.textContent),['Preview Mix','Apply Mix','Save + Pin','Apply + Lock']);
  const matchButtons=Array.from(matchApi.details.querySelectorAll('.inkframe-v2-brush-match-actions button'));assert.deepEqual(matchButtons.map(button=>button.textContent),['Preview Match','Load in Mixer','Apply Match','Save + Pin','Apply + Lock']);
  ...
  const stroke=async(id,points)=>{for(let i=0;i<points.length;i++){const p=points[i],type=i===0?'pointerdown':i===points.length-1?'pointerup':'pointermove';pointer(type,{pointerId:id,pointerType:'pen',clientX:p[0],clientY:p[1],pressure:p[2],timeStamp:p[3],buttons:type==='pointerup'?0:1});}await new Promise(r=>setTimeout(r,70));};
  ...
  const suggestion=coachApi.current(),initialMatch=matchApi.current();assert.equal(suggestion.valid,true);assert.ok(suggestion.confidence>=.55);assert.equal(initialMatch.valid,true);assert.equal(initialMatch.ranking.length,3);assert.ok(initialMatch.distance<=initialMatch.ranking[0].distance+1e-8);assert.equal(replayApi.stats().reference.available,true);assert.equal(reportApi.render().ready,2);
  ...
  assert.equal(mixerApi.setA('lovely-comet'),true);assert.equal(mixerApi.setB('precision-ink'),true);assert.equal(mixerApi.setPercent(50),50);assert.equal(mixerApi.current().tuning.stabilizerStrength,101);
  const mixBeforePreview=adapter.currentTuning(),presetCountBeforePreview=store.snapshot().presets.length;assert.equal(mixerApi.preview(),true);await new Promise(r=>setTimeout(r,40));assert.equal(preview.compareChoice().kind,'transient');assert.equal(preview.compareChoice().tuning.stabilizerStrength,101);assert.equal(store.snapshot().presets.length,presetCountBeforePreview,'transient mix preview must not create a preset');assert.equal(adapter.currentTuning().stabilizerStrength,mixBeforePreview.stabilizerStrength,'mix preview must not apply');assert.ok(replayApi.stats().lastReplay.b>0);
  const beforeSwap=JSON.stringify(mixerApi.current().tuning);mixerApi.swap();assert.equal(JSON.stringify(mixerApi.current().tuning),beforeSwap,'swapping a 50/50 mix must preserve its tuning');
  const savedMix=mixerApi.saveAndPin();assert.ok(savedMix&&savedMix.name.startsWith('Mix 50'));assert.ok(store.snapshot().pinned.includes(savedMix.id));
  assert.equal(mixerApi.apply(),true);assert.equal(adapter.currentTuning().stabilizerStrength,101);
  assert.equal(mixerApi.setA('animation-cleanup'),true);assert.equal(mixerApi.setB('fast-gesture'),true);assert.equal(mixerApi.setPercent(25),25);assert.equal(mixerApi.applyAndLock(),true);assert.equal(adapter.currentTuning().stabilizerStrength,119.5);assert.equal(recoveryApi.model.snapshot().locked.stabilizerStrength,119.5);

  const matched=matchApi.refresh(),matchTuning=JSON.stringify(matched.tuning),matchPresetCount=store.snapshot().presets.length,matchBeforePreview=JSON.stringify(adapter.currentTuning());assert.equal(matched.valid,true);assert.equal(matchApi.preview(),true);await new Promise(r=>setTimeout(r,40));assert.equal(preview.compareChoice().kind,'transient');assert.equal(JSON.stringify(preview.compareChoice().tuning),matchTuning);assert.equal(store.snapshot().presets.length,matchPresetCount,'Brush Match preview must not create a preset');assert.equal(JSON.stringify(adapter.currentTuning()),matchBeforePreview,'Brush Match preview must not apply');assert.ok(replayApi.stats().lastReplay.b>0);
  assert.equal(matchApi.loadInMixer(),true);assert.equal(mixerApi.current().a.id,matched.mix.a.id);assert.equal(mixerApi.current().b.id,matched.mix.b.id);assert.equal(mixerApi.current().percent,matched.mix.percent);assert.equal(JSON.stringify(mixerApi.current().tuning),matchTuning);
  const savedMatch=matchApi.saveAndPin();assert.ok(savedMatch&&savedMatch.name.startsWith('Match '));assert.ok(store.snapshot().pinned.includes(savedMatch.id));
  assert.equal(matchApi.apply(),true);assert.equal(JSON.stringify(adapter.currentTuning()),matchTuning);
  const nextMatch=matchApi.current(),nextMatchTuning=JSON.stringify(nextMatch.tuning);assert.equal(matchApi.applyAndLock(),true);assert.equal(JSON.stringify(adapter.currentTuning()),nextMatchTuning);assert.equal(JSON.stringify(recoveryApi.model.snapshot().locked),nextMatchTuning);

  assert.equal(recoveryApi.model.snapshot().entries.length<=24,true);assert.equal(recoveryApi.projectCanvasWrites,0);assert.equal(recoveryApi.undoWrites,0);assert.equal(observerApi.projectCanvasWrites,0);assert.equal(observerApi.undoWrites,0);assert.equal(identityApi.projectCanvasWrites,0);assert.equal(identityApi.undoWrites,0);assert.equal(mixerApi.projectCanvasWrites,0);assert.equal(mixerApi.undoWrites,0);assert.equal(matchApi.projectCanvasWrites,0);assert.equal(matchApi.undoWrites,0);
  ...
  dom.window.close();console.log('✅ generated Android Coach, recovery, identities, mixer, and Brush Match are project-isolated');
