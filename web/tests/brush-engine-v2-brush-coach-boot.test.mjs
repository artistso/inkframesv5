...
try{
  ...
  html=html.replace('</body>',`<script>${replay}</script><script>${coach}</script><script>${sessionSource}</script><script>${reportSource}</script><script>${recoverySource}</script><script>${observerSource}</script></body>`);
  ...
  const d=dom.window.document,preview=dom.window.InkFrameBrushV2PreviewPad,replayApi=dom.window.InkFrameBrushV2ReferenceReplay,coachApi=dom.window.InkFrameBrushCoach,sessionApi=dom.window.InkFrameBrushCoachSession,reportApi=dom.window.InkFrameBrushCalibrationReport,recoveryApi=dom.window.InkFrameBrushProfileRecovery,observerApi=dom.window.InkFrameBrushProfileRecoveryObserver,adapter=dom.window.InkFrameBrushV2Adapter;
  assert.ok(coachApi&&coachApi.installed,'Brush Coach did not install');assert.ok(sessionApi&&sessionApi.installed,'Coach Session did not install');assert.ok(reportApi&&reportApi.installed,'Calibration Report did not install');assert.ok(recoveryApi&&recoveryApi.installed,'Profile Recovery did not install');assert.ok(observerApi&&observerApi.installed,'Profile Recovery Observer did not install');
  ...
  const positionRow=Array.from(d.querySelectorAll('.inkframe-v2-tune-row')).find(row=>row.querySelector('span')?.textContent==='Position lag'),positionInput=positionRow&&positionRow.querySelector('input');assert.ok(positionInput,'legacy Position lag slider missing');const beforeLegacy=recoveryApi.model.snapshot().entries.length;positionInput.value='14';positionInput.dispatchEvent(new dom.window.Event('input',{bubbles:true,cancelable:true}));await new Promise(r=>setTimeout(r,30));assert.equal(Math.round(adapter.currentTuning().positionTimeConstantMs),14);assert.equal(recoveryApi.model.snapshot().entries.length,beforeLegacy+1,'legacy slider must create exactly one recovery entry');
  ...
  assert.equal(recoveryApi.model.snapshot().entries.length<=24,true);assert.equal(recoveryApi.projectCanvasWrites,0);assert.equal(recoveryApi.undoWrites,0);assert.equal(observerApi.projectCanvasWrites,0);assert.equal(observerApi.undoWrites,0);
  ...
  dom.window.close();console.log('✅ generated Android Coach, calibration, profile recovery, and legacy controls are project-isolated');
