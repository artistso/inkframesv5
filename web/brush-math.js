...
// Circular Canvas v4 prototype. This runs only in real browsers/WebView, not Node/jsdom smoke.
(function installCircularCanvasPrototype(){
  ...
  function ensureStyle(){
    ...
    style.textContent = [
      ...
      '#inkframe-playhead-bead{position:absolute;pointer-events:none;z-index:15;opacity:0;width:18px;height:18px;margin:-9px 0 0 -9px;border-radius:50%;transition:opacity .18s ease,left .16s ease,top .16s ease,transform .16s ease;background:radial-gradient(circle at 38% 32%,#fff 0%,#fff0f3 22%,#ff7bb5 48%,#bb0037 78%,rgba(42,0,26,.95) 100%);box-shadow:0 0 0 2px rgba(255,255,255,.92),0 0 18px rgba(187,0,55,.95),0 8px 18px rgba(20,0,14,.45)}',
      'body.circular-canvas #inkframe-timeline-ring,body.circular-canvas #inkframe-timeline-ticks,body.circular-canvas #inkframe-timeline-center,body.circular-canvas #inkframe-playhead-bead{opacity:1}',
  ...
  function restoreSquareBoard(){
    slots().forEach(slot => {
      ...
      slot.style.zIndex = slot.dataset.squareZIndex || '';
  ...
  function computeOrbit(cRect, fgRect, total){
    ...
    const density = total > 144 ? 2.85 : total > 96 ? 2.45 : total > 64 ? 2.15 : total > 40 ? 1.9 : 1.68;
    ...
    const maxOrbit = Math.max(canvasRadius + 2, safeBounds - idealSlot * 0.58);
  ...
  function layoutCircularBoard(){
    ...
    const tickStep = Math.max(3, 360 / Math.min(total, 120));
    ...
    if (ticks) {
      ticks.style.left = (orbit.cx - orbit.ringRadius) + 'px';
      ticks.style.top = (orbit.cy - orbit.ringRadius) + 'px';
      ticks.style.width = orbitSize + 'px';
      ticks.style.height = orbitSize + 'px';
      ticks.style.setProperty('--inkframe-tick-step', tickStep + 'deg');
    }
    if (center) {
    ...
    if (bead) {
      bead.style.left = (orbit.cx + Math.cos(beadAngle) * beadRadius) + 'px';
      bead.style.top = (orbit.cy + Math.sin(beadAngle) * beadRadius) + 'px';
      bead.style.transform = 'scale(' + clamp(0.78, orbit.idealSlot / 18, 1.12).toFixed(2) + ')';
    }
    ...
    list.forEach((slot, i) => {
      ...
      slot.style.zIndex = isCur ? '16' : isSel ? '15' : '13';
