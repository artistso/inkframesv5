...
function installVelocityRuntime(root) {
  ...
  function activateSample(event) {
    ...
    runtime.samples++;
    root.__inkframeVelocity = {
      brush:runtime.brushId,
      speed,
      amount:runtime.current.amount,
      width:runtime.current.width,
      opacity:runtime.current.opacity,
      flow:runtime.current.flow,
      spacing:runtime.current.spacing,
  ...
  function decorate(event) {
    ...
    return new Proxy(event, {
      get(target, prop) {
        if (!activated && (prop === 'clientX' || prop === 'clientY' || prop === 'pressure')) {
          activated = true;
          activateSample(target);
        }
  ...
  // Wrap the coalesced stream after brush-math has rejected spikes and normalized
  // cadence. Property access activates the matching dynamics before each dab.
  ...
  // Scope Canvas interception to the app's main pointermove listener. Other UI,
  // thumbnails, exports, playback, and idle renders retain normal Canvas behavior.
  ...
  eventProto.addEventListener = function(type, listener, options) {
    if (type === 'pointermove' && this && this.id === 'c' && typeof listener === 'function') {
      const wrappedListener = function(event) {
        ...
        finally { runtime.active = false; runtime.current = null; }
      };
      return originalAdd.call(this, type, wrappedListener, options);
    }
    if ((type === 'pointerup' || type === 'pointercancel') && typeof listener === 'function') {
      const wrappedEnd = function(event) {
        if (event && event.pointerId != null) runtime.previous.delete(event.pointerId);
  ...
  const originalEllipse = ctxProto.ellipse;
  ...
  function contextState(ctx) {
    let state = runtime.contexts.get(ctx);
    if (!state) { state={budget:1,skip:false}; runtime.contexts.set(ctx,state); }
    return state;
  }

  ctxProto.translate = function(x,y) {
    if (runtime.active && runtime.current) {
      const state = contextState(this);
      const spacing = Math.max(1, runtime.current.spacing);
      state.budget += 1 / spacing;
      if (state.budget >= 1) { state.budget -= 1; state.skip=false; }
      else state.skip=true;
    }
    return originalTranslate.call(this,x,y);
  };

  ctxProto.arc = function(x,y,r,start,end,ccw) {
    const mult = runtime.active && runtime.current ? runtime.current.width : 1;
    return originalArc.call(this,x,y,r*mult,start,end,ccw);
  };

  if (typeof originalEllipse === 'function') {
    ctxProto.ellipse = function(x,y,rx,ry,rotation,start,end,ccw) {
      const mult = runtime.active && runtime.current ? runtime.current.width : 1;
      return originalEllipse.call(this,x,y,rx*mult,ry*mult,rotation,start,end,ccw);
    };
  }
  ...
  for (const name of paintMethods) {
    ...
    ctxProto[name] = function(...args) {
      if (!runtime.active || !runtime.current) return original.apply(this,args);
      const state = contextState(this);
      if (state.skip) return undefined;
      const before = this.globalAlpha;
      this.globalAlpha = before * runtime.current.opacity * runtime.current.flow;
      try { return original.apply(this,args); }
      finally { this.globalAlpha = before; }
    };
  }

  root.InkFrameVelocityRuntime = {
    stats:()=>({samples:runtime.samples, current:root.__inkframeVelocity||null}),
    reset:()=>{runtime.samples=0;runtime.previous.clear();root.__inkframeVelocity=null;},
  };
  return true;
}

{
  const api = {
    ...
    sampleSpeed,
    installVelocityRuntime,
  };
  if (typeof window !== 'undefined') {
    window.InkFrameBrushDynamics = api;
    installVelocityRuntime(window);
  }
