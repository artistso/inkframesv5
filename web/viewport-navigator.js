...
(function(root){
  ...
  function render(value){
    ...
    if(!state){navigator.hidden=true;return false;}
    ...
    navigator.title=`Canvas navigator · ${visiblePercent}% of canvas visible · drag to recenter`;
  ...
  function recenterAt(clientX,clientY){
    ...
    const applied=env.setState({
      scale,
      panX:-(canvasX-canvasWidth/2)*scale,
      panY:-(canvasY-canvasHeight/2)*scale,
    });
  ...
  function onPointerEnd(event){
    ...
    const id=event&&event.pointerId==null?'mouse':event&&event.pointerId;
    if(id!==pointerOwner)return;
  ...
  function install(){
    ...
    navigator.setAttribute('role','application');
  ...
  const exported=Object.freeze({geometry,render,recenterAt,install,get installed(){return installed;}});
