...
(function(root){
  ...
  function normalizeDescriptor(value){
    ...
    return Object.freeze({
      tag:safe(input.tag||input.tagName).toLowerCase(),id:safe(input.id).toLowerCase(),
      text:safe(input.text||input.textContent||input.label),title:safe(input.title),
      classes:Object.freeze(classTokens(input.className||input.classes)),disabled:!!input.disabled||ariaDisabled==='true',
      ariaPressed:safe(input.ariaPressed||input['aria-pressed']).toLowerCase(),action:data(input.dataset,'action'),
      timelineCommand:data(input.dataset,'timelineCommand'),layerCommand:data(input.dataset,'layerCommand'),
    });
  }
  ...
  function classifyDescriptor(value){
    ...
    const radial=classes.has('orb')||classes.has('kid'),frame=classes.has('frameSlot'),exactText=item.text.toLowerCase();
  ...
  function descriptorFromElement(element){return {
    tagName:element&&element.tagName,id:element&&element.id,textContent:element&&element.textContent,title:element&&element.title,
    className:element&&element.className,dataset:element&&element.dataset,disabled:!!(element&&element.disabled),
  ...
  function applyControlState(element){
    ...
    for(const [name,on] of Object.entries({
      'ink-control--radial':state.radial,'ink-control--frame':state.frame,'ink-control--transport':state.transport,
      'ink-control--danger':state.danger,'ink-control--selected':state.selected,'ink-control--compact':state.compact,
...
.ink-control{min-height:var(--ink-control-min)!important;border:1px solid var(--ink-control-border)!important;border-radius:var(--ink-control-radius)!important;background:var(--ink-control-bg)!important;color:var(--text,#fff0f3)!important;box-shadow:var(--ink-control-shadow)!important;font-family:var(--font-ui,system-ui,sans-serif)!important;font-weight:800!important;letter-spacing:.045em!important;text-shadow:0 1px 2px rgba(0,0,0,.62)!important;-webkit-tap-highlight-color:transparent;transition:background .16s ease,border-color .16s ease,box-shadow .16s ease,filter .16s ease,opacity .16s ease,transform .10s ease!important}
...
.ink-control--frame{border-radius:7px!important}.ink-control--transport{border-color:rgba(247,202,201,.42)!important}.ink-control--danger{background:var(--ink-control-bg-danger)!important;border-color:rgba(255,145,165,.48)!important}
...
@media(hover:hover){.ink-control:hover,.ink-control.inkframe-pen-hover{background:var(--ink-control-bg-hover)!important;border-color:rgba(255,240,243,.52)!important;box-shadow:var(--ink-control-shadow-hover)!important;filter:brightness(1.06)}button.ink-control:hover:not(.ink-control--radial):not(.ink-control--frame),[role="button"].ink-control:hover:not(.ink-control--radial):not(.ink-control--frame){transform:translateY(-1px)}}
...
.ink-control:focus-visible{outline:3px solid rgba(255,240,243,.94)!important;outline-offset:3px!important}.ink-control:disabled,.ink-control[data-inkframe-disabled]{opacity:.38!important;filter:saturate(.35)!important;box-shadow:none!important;cursor:not-allowed!important}
#inkframeTabletDeck .deck-modes button,#inkframeTabletDeck .deck-transport button,#inkframeTabletDeck .deck-utilities button,#inkframeTimelineWorkspace button,#inkframeLayerWorkspace button{border-radius:13px!important;text-transform:uppercase!important}#inkframeTabletDeck .deck-icon,#inkframeTabletDeck .deck-grip{border-radius:50%!important}
...
@media(pointer:coarse){button.ink-control:not(.ink-control--compact),[role="button"].ink-control:not(.ink-control--compact){min-height:var(--ink-control-min-coarse)!important}#inkframeTabletDeck .deck-icon,#inkframeTabletDeck .deck-grip{width:52px!important;height:52px!important;min-height:52px!important}.frameSlot.ink-control{min-width:28px!important;min-height:28px!important}}
...
`;}
  ...
  function installStyle(document){
    if(!document||!document.head)return false;if(document.getElementById(STYLE_ID))return true;
    const style=document.createElement('style');style.id=STYLE_ID;style.dataset.inkframeControlSurface='v2';style.textContent=css();document.head.appendChild(style);return true;
  }
  function queueRefresh(){
    if(refreshQueued)return;refreshQueued=true;
    const run=()=>{refreshQueued=false;const document=root&&root.document;if(document)decorateTree(document);};
    if(root&&typeof root.requestAnimationFrame==='function')root.requestAnimationFrame(run);
    else if(root&&typeof root.setTimeout==='function')root.setTimeout(run,0);else run();
  }
  ...
  function clearPressed(event){const control=controlFromEvent(event);if(control)control.classList.remove('is-pressed');queueRefresh();}

  function install(){
    const document=root&&root.document;if(!document)return false;installStyle(document);decorateTree(document);
    if(document.addEventListener&&!document.documentElement.dataset.inkframeControlEvents){
      ...
      document.addEventListener('click',queueRefresh,true);document.addEventListener('keyup',queueRefresh,true);
      document.addEventListener('change',queueRefresh,true);document.addEventListener('input',queueRefresh,true);
    }
    if(typeof root.MutationObserver==='function'&&!observer){
      ...
      observer.observe(document.documentElement,{subtree:true,childList:true,attributes:true,attributeFilter:['disabled','aria-pressed','title']});
    }
    return true;
  }
  ...
  if(root&&typeof root.addEventListener==='function'){
    root.addEventListener('load',scheduleInstall);root.addEventListener('orientationchange',scheduleInstall);
    root.addEventListener('inkframe:timeline',queueRefresh);root.addEventListener('inkframe:layers',queueRefresh);
  }
  scheduleInstall();
