...
(function(root){
  ...
  function loadProfileRecoveryObserver(){
    if(!root.document||root.InkFrameBrushProfileRecoveryObserver)return false;
    if(root.document.querySelector('script[data-inkframe-profile-recovery-observer]'))return true;
    const script=root.document.createElement('script');script.src='brush-engine-v2/profile-recovery-observer.js';script.async=false;script.dataset.inkframeProfileRecoveryObserver='true';root.document.head.appendChild(script);return true;
  }
