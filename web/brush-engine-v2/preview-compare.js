...
(function(root){
  ...
  function loadProfileHistory(){
    if(!root.document||root.InkFrameBrushProfileHistory)return false;
    if(root.document.querySelector('script[data-inkframe-profile-history]'))return true;
    const script=root.document.createElement('script');script.src='brush-engine-v2/profile-history.js';script.async=false;script.dataset.inkframeProfileHistory='true';root.document.head.appendChild(script);return true;
  }
  function loadBrushSignature(){
    if(!root.document)return false;
    if(root.InkFrameBrushSignature){root.setTimeout(loadProfileHistory,0);return true;}
    const existing=root.document.querySelector('script[data-inkframe-brush-signature]');if(existing){existing.addEventListener&&existing.addEventListener('load',()=>root.setTimeout(loadProfileHistory,0),{once:true});return true;}
    const script=root.document.createElement('script');script.src='brush-engine-v2/brush-signature.js';script.async=false;script.dataset.inkframeBrushSignature='true';script.addEventListener('load',()=>root.setTimeout(loadProfileHistory,0),{once:true});root.document.head.appendChild(script);return true;
  }
  ...
  function loadIdentityMixer(){
    if(!root.document)return false;
    if(root.InkFrameBrushIdentityMixer){root.setTimeout(loadBrushMatch,0);return true;}
    const existing=root.document.querySelector('script[data-inkframe-identity-mixer]');if(existing){existing.addEventListener&&existing.addEventListener('load',()=>root.setTimeout(loadBrushMatch,0),{once:true});return true;}
    const script=root.document.createElement('script');script.src='brush-engine-v2/identity-mixer.js';script.async=false;script.dataset.inkframeIdentityMixer='true';script.addEventListener('load',()=>root.setTimeout(loadBrushMatch,0),{once:true});root.document.head.appendChild(script);return true;
  }
  function loadProfileIdentities(){
    if(!root.document)return false;
    if(root.InkFrameBrushProfileIdentities){root.setTimeout(loadIdentityMixer,0);return true;}
    const existing=root.document.querySelector('script[data-inkframe-profile-identities]');if(existing)return true;
    const script=root.document.createElement('script');script.src='brush-engine-v2/profile-identities.js';script.async=false;script.dataset.inkframeProfileIdentities='true';script.addEventListener('load',()=>root.setTimeout(loadIdentityMixer,0),{once:true});root.document.head.appendChild(script);return true;
  }
