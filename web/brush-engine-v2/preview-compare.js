...
(function(root){
  ...
  function loadCalibrationReport(){
    ...
    if(root.InkFrameBrushCalibrationReport){root.setTimeout(loadProfileHistory,0);return true;}
    const existing=root.document.querySelector('script[data-inkframe-calibration-report]');if(existing)return true;
    const script=root.document.createElement('script');script.src='brush-engine-v2/calibration-report.js';script.async=false;script.dataset.inkframeCalibrationReport='true';script.addEventListener('load',()=>root.setTimeout(loadProfileHistory,0),{once:true});root.document.head.appendChild(script);return true;
  }
