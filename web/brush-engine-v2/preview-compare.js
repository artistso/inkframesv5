...
(function(root){
  ...
  function loadProfileRecovery(){
    if(!root.document||root.InkFrameBrushProfileRecovery)return false;
    if(root.document.querySelector('script[data-inkframe-profile-recovery]'))return true;
    const script=root.document.createElement('script');script.src='brush-engine-v2/profile-recovery.js';script.async=false;script.dataset.inkframeProfileRecovery='true';root.document.head.appendChild(script);return true;
  }
