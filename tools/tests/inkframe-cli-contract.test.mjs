...
const run=(args=[])=>spawnSync('bash',[cliPath,...args],{
  cwd:root,
  encoding:'utf8',
...
assert.doesNotMatch(combinedDocs,/\.\/inkframe-cli\s+export-gif\b/);
assert.doesNotMatch(combinedDocs,/\bagent\s+export-gif\b/);
assert.doesNotMatch(combinedDocs,/tools\/inkframe-cli\.mjs/);
assert.doesNotMatch(combinedDocs,/bit-identical to the in-app encoder/i);
...
const syntax=spawnSync('bash',['-n',cliPath],{cwd:root,encoding:'utf8'});
...
const before=spawnSync('git',['status','--porcelain','--untracked-files=all'],{cwd:root,encoding:'utf8'}).stdout;
for(const args of [
...
const after=spawnSync('git',['status','--porcelain','--untracked-files=all'],{cwd:root,encoding:'utf8'}).stdout;
assert.equal(after,before,'unsupported commands must not mutate the checkout');

console.log('✅ CLI help, unsupported-command failure, documentation, and no-mutation contracts passed');
