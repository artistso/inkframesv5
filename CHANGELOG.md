...
- **Monopoly-board perimeter timeline.** A new frame board wraps around the canvas outside edge, scales with canvas zoom, shows existing frames plus placeholder slots, lets users tap placeholders to add frames, and enforces a hard 120-frame cap to protect tablet memory.
